...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteReply = (e) => {
        ...
        // 삭제 요청 URL
        const reqUrl = `/mugmung/api/community/${id}`;
        // 삭제 요청을 Ajax 방식으로 보냄.
        axios.delete(reqUrl)
            .then((response) => {
                console.log(response);
                alert('댓글 삭제 성공');
                getRepliesWithId(); // 댓글 목록 갱신.
            })
            .catch((error) => {
                console.log(error);
            });
    ...
    const makeReplyElements = (data) => {
        ...
        for (let capture_reply of data) {
            console.log(capture_reply);
            
            // Timestamp 타입 값을 날짜/시간 타입 문자열로 변환:
            const modified = new Date(capture_reply.modifiedTime).toLocaleString();
            
            // 댓글 1개를 표시할 HTML 코드:
            htmlStr += `
            <div class="card">
                <div>
                    <span class="d-none">${capture_reply.capture_id}</span>
                    <span class="fw-bold">${capture_reply.writer}</span>
                    <span class="text-secondary">${modified}</span>
                </div>
                <div>
                    ${capture_reply.replyText}
                </div>
                <div>
                    <button class="btnDelete btn btn-outline-danger" data-id="${capture_reply.id}">
                        삭제
                    </button>
                    <button class="btnModify btn btn-outline-success" data-id="${capture_reply.id}">
                        수정
                    </button>
                </div>
            </div>
    ...
    const getRepliesWithId = async () => {
        
        console.log('123');
        // 댓글 목록을 요청하기 위한 포스트 번호(아이디)
        const id = document.querySelector('input#id').value;
        // 댓글 목록을 요청할 URL
        const reqUrl = `/mugmung/api/community/all/${id}`;
        console.log('reqUrl : '+reqUrl);
    ...
    btnToggleReply.addEventListener('click', () => {
        ...
        if (toggleBtnIcon.alt === 'toggle-off') {
             toggleBtnIcon.src = '/mugmung/static/assets/icons/toggle-on.png';
             toggleBtnIcon.alt = 'toggle-on';
             
             // 댓글 전체 목록을 서버에 요청하고, 응답이 오면 화면 갱신.
             getRepliesWithId();
        } else {
            toggleBtnIcon.src = '/mugmung/static/assets/icons/toggle-off.png';
    ...
    const createReply = (e) => {
        const id = document.querySelector('input#id').value;
        const replyText = document.querySelector('textarea#reply_text').value;
        ...
        const data = { capture_id: id, reply_text: replyText, writer, };
        
        axios.post('/mugmung/api/community', data) // POST 방식의 Ajax 요청 보냄.
            .then((response) => {
                alert(`댓글 등록 성공(${response.data})`);
                
                // 댓글 입력 창의 내용을 지움.
                document.querySelector('textarea#reply_text').value = '';
